...
We still leave a conceptual overview of the tutorial in simpler language because we do want readers of the paper to come away with some idea of what making a theory FAIR entails. Note that these general principles are neither tied to the specific R package nor to our specific implementation. Importantly, they could be implemented using an entirely different infrastructure.
...
The second is that our work is reminiscent of Meehl's nomological nets.
This is definitely true for the way that *we* are using FAIR theory (e.g., in the online examples), as we are inspired by Meehl,
