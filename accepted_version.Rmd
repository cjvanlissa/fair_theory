...
authornote: |
  We are particularly indebted to one of our Reviewers, Dr. Eric Turkheimer, whose thoughtful reflections on FAIR theory were incorporated into the Conclusion. We also thank the Editor, Dr. Scott Vrieze, and two anonymous Reviewers for their detailed and invaluable comments. Our manuscript is reproducible given the code provided in our GitHub repository identified by the snapshot with the unique commit ID #`r substr(gert::git_commit_id(),1,8)`. The first and senior author positions were fixed, and remaining author positions randomized.
...
<span id="alldefinitions">Definitions of theory abound and are hotly debated,
but as many are compatible with the FAIR principles,
this paper is not limited to one particular definition.</span>
...
We recognize the syntactic view in Meehl's [-@meehlAppraisingAmendingTheories1990, p. 114] hierarchy of ever-more specific "statements" a theory might contain:
...
<span id="figure1b">Wagenmakers and colleagues divided this cycle into two phases [-@wagenmakersCreativityVerificationCyclePsychological2018, Figure \@ref(fig:figecs)b].
...
in which patterns observed in data are generalized to theoretical principles.</span>
<span id="introduce1c">In these interpretations of the empirical cycle, each stage describes a process.
...
In this specification, `theory` refers to FAIR theory,
`hypothesis` to machine-readable hypotheses [@lakensImprovingTransparencyFalsifiability2021],
`test` refers to a preregistered inferential procedure [Preregistration-As-Code, @peikertReproducibleResearchTutorial2021],
data refers to FAIR data,
and results refer to (preprint) papers, ideally supplemented with a comprehensive reproducible research archive [@vanlissaWORCSWorkflowOpen2021].
In this model, theories are the vehicle of scientists' understanding of phenomena;
they are what changes when iterating through the cycle.</span>
...
# rsvg::rsvg_pdf("empirical_cycle3.svg", "ec_visio.pdf")
...
The DataCite Metadata Schema provides a controlled vocabulary for research output [@datacitemetadataworkinggroupDataCiteMetadataSchema2024].
For example, data are typically archived with the metadata property `resourceType: dataset`.
The `resource_type: model` would be more appropriate for archiving FAIR theory.
If data were collected for a specific paper, that relationship can be cross-referenced with `relationType: IsSupplementTo`.
Similarly, a FAIR theory can be connected to a theory paper using `relationType: IsDescribedBy`,
while the reverse relationship, documented in the theory paper, is `relationType: Describes`.
...
Other relevant metadata might be a reference to a taxonomy of psychological constructs [@boscoMetaBUSVehicleFacilitating2017],
...
We propose using the keyword `"fairtheory"` for all resources that constitute or reference (a specific) FAIR theory,
...
The `"fairtheory"` keyword can also be used in print publications to signal their association with a FAIR theory.
...
although it is advisable to make use of automatic integration (as exists between the [GitHub](http://github.com/), [Zenodo](https://zenodo.org), and [Open Science Foundation (OSF)](https://osf.io) platforms)
...
Such platforms include [GitHub](https://github.com), which is well-suited for collaborative theory development, and [Zenodo](https://zenodo.org), which is better suited for long-term storage of the version of record.
The resulting resource will have an identifier (DOI) which allows it to be accessed using a standardized communications protocol (download via `https` or `git`).
...
<!-- Note that Accessibility does not always mean that a resource is unconditionally accessible to all. -->
<!-- On platforms like GitHub and Zenodo, access can be regulated, . -->
...
This knowledge can come from shared paradigms [@kuhnStructureScientificRevolutions2009],
...
If the same theories are instantiated in the DOT graph description language [@DOTLanguage2024], then they can be displayed, resized, and edited before being printed without loss of quality.
...
If we consider Meehl's nine levels of strong theories [-@meehlAppraisingAmendingTheories1990, p. 114],
...
<!--  FAIR theories may be X-Interoperable for various operations, -->
<!-- including deriving testable hypotheses, -->
<!-- selecting relevant control variables, -->
<!-- and indicating the conditions necessary for observing a particular phenomenon. -->
...
Kurt Lewin's [-@lewinPsychologyProcessGroup1943] adage "there's nothing as practical as a good theory" paints a hopeful picture of theories as useful tools in psychological researchers' daily workflows.
...
A final suggestion is to create or adopt a "Code of Conduct" which prescribes behavioral norms for contributors and users of a theory, for example, the [Berlin Code of Conduct](https://berlincodeofconduct.org/en).
...
<span id="phonoloop">Take for example the phonological loop in Baddeley's working memory model [@baddeleyWorkingMemory1992]. The idea behind it is that verbal information is kept in memory via a rehearsal loop with limited capacity. The verbal description of the phonological loop in Baddeley's theory of working memory stands out for clarity and comprehensibility, yet it allows for at least 144 different implementations depending on the specification of various parameters such as decay rate of information, recall success, or rehearsal sequence, which were left undefined in the original theory [@lewandowsky2010computational].</span>
...
We envision FAIR theory as another instance of modular publishing [@kirczModularityNextForm1998].
</span>

Modular publishing can be achieved by archiving specific resources (including theory) in FAIR-compliant repositories like [Zenodo](https://zenodo.org/).
...
Cloud platforms associated with Git (e.g., GitHub) facilitate collaborative theory development, for example by making it possible to duplicate an entire theory archive and develop it in a new direction (technical term: *forking*),
suggesting changes to an existing theory archive (*pull request*),
and explicitly comparing differences between versions of a theory (*comparing commits*).
...
how to FAIRify Morris' [*Tripartite Model*](https://cjvanlissa.github.io/theorytools/articles/causal-inference.html) of parental socialization of children's emotions [@morrisRoleFamilyContext2007] and use it for causal inference,
...
Elaborating on our running example, we can implement Wagenmakers and colleagues' take on the empirical cycle [-@wagenmakersCreativityVerificationCyclePsychological2018, Figure \@ref(fig:figecs)b].
...
Thus in both prior versions, it is unclear *how* knowledge accumulates in the evaluation phase.
In Van Lissa's specification (2025), `theory` is understood as FAIR theory,
...
Theory development and amendment, as takes place in the "context of discovery", involves mostly inductive reasoning^[Here, "induction" is defined forming general theories based on specific observations. Others have used the term "abduction" to describe "inference to the best explanation" [@peirceCollectedPapersCharles1960]. For present purposes, the terms are interchangeable.].
However, deriving hypotheses from theory is not *entirely* deductive
as auxiliary assumptions must often be made to account for remaining ambiguities in theory, which involves induction [@peikertTransparencyOpenScience2023].
...
Additionally, if we accept that observation is theory-laden, then it too involves induction [@brewerTheoryLadennessObservationTheoryLadenness2001].
...
The FAIR theory workflow offers concrete ways to make changes to a FAIR theory object and to compare different versions, [as explained here](https://cjvanlissa.github.io/theorytools/articles/updating_theory.html).
...
Fourth, the availability of Open Educational Materials (OEM) about theory development contributes to doctoral socialization [@schonbrodtNicebreadFOMOPsyFormal2025; @vandongenCourseMaterialFundamentals2025].
...
<span id="nomological">In their seminal paper on construct validity and the use of nomological nets, Cronbach and Meehl [@cronbachConstructValidityPsychological1955] discuss crucial insights that are compatible with a FAIR theory approach.
...
then it would become possible to evaluate the entire body of empirical evidence (FAIR data) against these machine-readable tests in an ongoing manner - similar to a continuously cumulating meta-analysis.</span>
...
Preregistration, by contrast, allows scholars to eliminate inductive bias from hypothesis tests and increase trust in the outcomes of a specific empirical study [@peikertWhyDoesPreregistration2023].
...
<!-- ^[Note that preregistration is also used for other purposes, including for qualitative or exploratory studies. While many researchers find preregistration useful in these contexts, it is hotly debated whether ]. lakensBenefitsPreregistrationRegistered2024-->
...
<span id="evaluateimpact">Finally, while this paper has discussed the potential impact of FAIR theory in addressing contemporary challenges in psychology,
future research is needed to evaluate whether this potential is realized.
Registered reports were first introduced to psychology about a decade ago [@nosekRegisteredReportsMethod2014],
and are now becoming commonplace, as are studies evaluating their adoption and impact on scientific practices and research results [@scheelExcessPositiveResults2021].
We envision a similar trajectory for FAIR theory.</span>

## Conclusion

<span id = "turkheimer">How would adopting a FAIR framework for theory construction, improvement, and reuse affect scholarly workflows?
FAIR theory can be used to derive and justify hypotheses,
thus addressing the problem that most hypotheses in psychology are currently atheoretical [@mcphetresDecadeTheoryReflected2021].
...
The Taskforce Theoretical Psychology developed a template for theory papers that addresses this question [@vanlissaProceedingsTaskforceTheoretical2025].
...
Developing FAIR theory could be a new type of scholarly output, and should be recognized and rewarded as such according to the [Coalition or Advancing Research Assessment](https://coara.org/wp-content/uploads/2022/09/2022_07_19_rra_agreement_final.pdf).</span>
