...
A third, and related, impediment is a phenomenon we will call "dependency on the author" (DOA).
DOA occurs whenever a theory cannot be understood by independent scholars,
...
DOA is an obstacle when theory is insufficiently clear to derive hypotheses from it without first asking the author what they meant.
It also renders theories immune to refutation,
because the author can claim that the theory was misconstrued when confronted with falsifying evidence, thus making it a moving target.
DOA relates to the discourse on "Great Man Theorizing" [@guestWhatMakesGood2024] because dependency on the author creates a potential for gatekeeping - the author could insist that work requires their involvement or denounce work conducted outside their purview as illegitimate, which violates checks and balances of scientific research.

DOA is also a problem because authors may not understand their own theories well.
illustrated by instances where third parties identify logical inconsistencies within a theory [e.g., @kissnerIDENTIFICATIONLOGICALINCONSISTENCY2008].
...
This definition raises an interesting question, namely *interoperable for what?*
Suitable answers for FAIR theory may be: this theory is X-interoperable for deriving testable null-hypotheses, or for the purpose of selecting relevant control variables,
...
According to Kurt Lewin, "there's nothing as practical as a good theory" [@lewinPsychologyProcessGroup1943] - but as we argued bfore,
there are clear signs that theory rarely plays a practical role in the work of contemporary psychologists.
Interoperability makes it possible to integrate theory into scientists' day-to-day work in a practical way.
If a theory is X-interoperable with software used for data analysis,
it becomes possible to integrate theory directly into analysis workflows.
For example, interoperable theory could be used to derive machine-readable hypotheses [@lakensImprovingTransparencyFalsifiability2021],
to select control variables for causal inference [@cinelliCrashCourseGood2022],
and to preregister the inferential procedure that would lead to specific modifications of a theory after analyzing empirical data [@peikertReproducibleResearchTutorial2021]
Additionally, theories should be interoperable with each other:
for instance, it should be possible to embed a specific theory about the process of emotion regulation [e.g., @grossEmotionRegulationCurrent2015] within a theory of emotion regulation development [@morrisRoleFamilyContext2007].
...
some have described a norm *against* theory reuse:
*"[Theories are] like toothbrushes — no self-respecting person wants to use anyone else's"* [@mischelToothbrushProblem2008].
Given that theory is the vehicle of cumulative knowledge acquisition [@degrootMethodologieGrondslagenVan1961],
such a norm impedes scientific progress.
Even if scholars wish to diverge substantially from prior theory,
explicitly refering back to it enables clear comparison of the differences [@ramGitCanFacilitate2013].
Reusing and adapting existing theories makes it possible to study the provenance of a theory and investigate how well different iterations of the theory explain emprical evidence [@vanlissaUsingEndpointsCheck2023]
...
These questions suggest that there is a norm against modifying theory without consent from the author.
However, scholars might not be aware of the legal limits to the rights authors can claim over theory.
An important concept in copyright law is the *idea-expression dichotomy* [@bently2010copyright].
...
Applying these principles to FAIR theory, it seems that vague, ambiguous verbal explanations of theories are more likely to qualify for copyright protection than formal theories.
If copyright does not cover (formal) theory,
then it would be pointless to adopt a license that assumes copyright protection.
Furtermore, even if copyright would apply, academic research is covered under "fair use" exemptions to copyright law,
so copyright would impose few restrictions to theory reuse in scholarly communication.
...
Simultaneously, to comply with the interoperability principle, the principle of comprehensive citation [@aalbersbergMakingScienceTransparent2018],
