...
One main concern is that theories lack precision, or  *formalization* [@szollosiArrestedTheoryDevelopment2021].
...
*Interoperable* for specific purposed, for example, within the data analysis environment;
...
Greater formalization increases theories' *empirical content* [@popperLogicScientificDiscovery2002] because it expresses ideas as precise statements, clearly demarcating what should (not) be observed.
...
Similarly, a FAIR theory object can be connected to a specific paper which might serve as the theory's documentation and canonical reference by using `relationType: IsDescribedBy`, while the reverse relationship, documented in the canonical reference paper, is `relationType: Describes`. Other types are useful for making relationships between multiple theory objects explicit: If an existing narrative theory is FAIRified without much own contribution, the FAIR theory would cross-reference the paper with the narrative theory as `relationType: IsDerivedFrom`; if substantial own contributions were necessary, `relationType:IsNewVersionOf` could be used. If a variation of an existing FAIR theory is created, cross-reference it with `relationType: IsVariantFormOf`.
...
Furthermore, we suggest using the keyword `"fairtheory"` for all resources that constitute or reference (a) FAIR theory.
...
although it is advisable to make use of automatic integration (as exists between GitHub, Zenodo, and OSF) to avoid the need to maintain information in multiple places and prevent inconsistencies from arising.
...
In reality, however, paradigms are rarely clear-cut, and shared background knowledge can come from different sources,
...
these considerations underscore the importance of being as explicit as possible about assumptions and ontological definitions when making theory Accessible,
...
formalization,
and explicit cross-references to relevant resources (papers, ontologies, macro-theories, measurement instruments, experimental designs) [REF langeChecklist2025].
...
Kurt Lewin's adage "there's nothing as practical as a good theory" [@lewinPsychologyProcessGroup1943] implies that theories ought to be highly X-Interoperable in psychological researchers' day-to-day work.
...
[This is an example of FAIR metadata](https://raw.githubusercontent.com/cjvanlissa/fair_theory/refs/heads/main/DGPSabstract.md) extracted from Zenodo.
...
These questions imply a norm against modifying theory without consent from the author. <!-- NvD: I suggest removing the rest of the sentence (as a result of an earlier suggestion). -- > reminiscent of the aforementioned problem of dependency on the author.
...
the norms of good scientific practice require that scholars comprehensively cite theory and related works [@aalbersbergMakingScienceTransparent2018].
...
and the integration of GitHub and Zenodo makes for a particularly user-friendly approach that meets *all* of the FAIR principles (Table \@ref(tab:tabfair)).
...
the approach proposed here is subject to change.
Nevertheless, it illustrates general principles which can also be implemented using other open science infrastructures.
Our workflow can be largely automated in R using the `theorytools` package.
For the most recent version of our proposed workflow, see the package vignette on FAIR theory, `vignette("fairtheory", package = "theorytools")`.
...
Note that this function also automatically submits basic FAIR theory metadata to Zenodo.
...
<!-- ``` -->
For example, many have argued that observation is theory-laden, and as such, involves induction [@brewerTheoryLadennessObservationTheoryLadenness2001].
...
Some have argued that *causal explanations* are a property of good theory [@meehlAppraisingAmendingTheories1990].
