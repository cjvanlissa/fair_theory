...
rsvg_svg(charToRaw(export_svg(ec_wagenmakers)), "graph.svg")
```
...
we suggest using semantic versioning for both, starting with version `0.1.0`,  unless you use another convention for versioning (which should be documented in README.md).
...
but closer examination reveals that not all of them have interpreted his work in the same way.
...
One advantage of FAIR theory is that we can take work that has been done before - in this case, the repository created above, and create an independent copy that we can modify as we wish, while retaining cross-references to the original.
This illustrates an important advantage of FAIR theory:
we can implement different versions of a theory, and study their relationships.
In practice, this is achieved by ["forking the repository"](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo),
...
We attempted to implement Wagenmakers and colleagues' version as a DOT graph,
to illustrate some clear deviations from the original.
The authors deviate from De Groot in a minor way by renaming the five phases. 
While this change was not made explicit (thus illustrating an advantage of FAIR theory), we assumed that the phases are identical, just with different labels.
The authors do mention one explicit change: *"We added the Whewell-Peirce-Reichenbach distinction between the context of discovery and the context of justification"*.
The DOT graph below illustrates this change:
