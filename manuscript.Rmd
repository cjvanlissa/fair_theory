...
The following conceptual workflow does not require the use of R, but most steps can be automated in R using the `theorytools` package.
The package also includes a living document with the [latest version of the workflow](https://cjvanlissa.github.io/theorytools/articles/fair-theory.html).
...
See here for [an example of the metadata](https://raw.githubusercontent.com/cjvanlissa/fair_theory/refs/heads/main/example_metadata.json) associated with our FAIR empirical cycle, as archived in Zenodo.
...
- Optionally, add *References* to related works in plain text. For example, we cite De Groot and Spiekerman in this field.
...
For example, the DOT graph below implements Wagenmakers and colleagues' [-@wagenmakersCreativityVerificationCyclePsychological2018] interpretation of the empirical cycle.
...
Figure \@ref(fig:figgithub-1) shows how our example `empirical_cycle` repository contains branches with Wagenmakers' and Van Lissa's implementations.
...
```{r figgithub, out.width = "50%", fig.show='hold', fig.cap='FAIR Theories on GitHub', fig.subcap=c('Branches contain different versions of a theory', 'These versions can be compared'), fig.ncol = 2}
#par(mfrow=c(1,2))
