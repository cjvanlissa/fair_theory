...
The present paper advances transparent scholarly communication by applying the FAIR principles to scientific theories.
...
In order to make theory FAIR, it must be instantiated as a type of data,
in a format that is both human- and machine-readable with as few interpretative steps as possible.
Verbal statements in plain text fit this description,
as do mathematical formulae and statements expressed in some axiomatic system.
Examples include pseudo-code,
interpretable computer code,
graphs described in the DOT language,
and Gray's theory maps [@grayHowMapTheory2017].
The common practice of instantiating theory as a bitmap image reproduced in a journal article falls short of this ideal;
but the same image represented as a DOT graph (which can in turn be *reproduced* in the print version) does meet this ideal.
Assuming that the theory is instantiated as human- and machine readable data, we can apply the principles of FAIR data to it.
