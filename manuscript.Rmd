...
authornote: |
  This is a preprint paper, generated from Git Commit # `r substr(gert::git_commit_id(),1,8)`. The first and senior author positions were fixed, and remaining author positions randomized using `set.seed(6999982); sample(c("Aaron", "Felix", "Noah", "Maximilian"))`.
...
was caused by undisclosed flexibility in analyses led to widespread adoption of open science practices like preregistration and replication [@nosekPromotingOpenResearch2015a].
...
Modular publishing is the practice of making each of these resources available as independent citable *information artifacts*,
...
and posted many examples on their website <www.theorymaps.org> [@grayHowMapTheory2017].
...
It has been proposed that good theories have the property of "discursive survival [...], the ability to be understood" [@guestWhatMakesGood2024]. 
...
Interoperability pertains to the property of information artifacts to "integrate or work together [...] with minimal effort" [@wilkinsonFAIRGuidingPrinciples2016].
...
An important aspect of Interoperability lies in linking FAIR theories with existing open science methodologies---most notably, preregistration.
As noted, preregistration alone has not resolved the replication crisis in the social sciences.
This raises a fundamental question: What additional value does a FAIR theory provide, and how does it complement preregistration?

One distinction is quantitative:
FAIR theories operate at a higher level of abstraction.
They articulate general principles and expectations that cut across studies, whereas preregistrations focus on specific applications, analytic strategies, and study designs.
More fundamentally, however, there is a qualitative difference.
A FAIR theory is designed to make predictions and be revised in light of empirical results; preregistration, by contrast, prescribes how a particular study will be conducted, regardless of what the outcomes turn out to be.

This difference can be illustrated with a Bayesian analogy.
A preregistration is akin to a statistical model---it formalizes assumptions about data generation but is in itself indifferent to the data.
Combining a theory with a specific sampling strategy, study design, measurement, etc. allows one to make predictions even before observing data [@peikertWhyDoesPreregistration2023].
However, after observing data, the theory might require revision.
In this sense, a theory is first a prior, and then after conducting a specific investigation as prescribed by the preregistration, is refined to become a posterior.
In short, a preregistration rooted in a FAIR theory not only supports transparency, but also strengthens the epistemic cycle of prediction, testing, and revision---moving us closer to a cumulative science.
