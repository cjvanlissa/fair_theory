...
@scientifictheories describe different schools of thought when it comes to scientific theories: the syntactic, the semantic, and the pragmatic view.
The syntactic view describes theories as "sets of sentences in a given logical domain language" [@scientifictheories, ch. 2], where domain refers to the scientific field (such as psychology or physics), with each domain having it's own theoretical terms (such as "intelligence" in psychology).
...
The semantic view challenges the necessity of distinct domain languages for different scientific fields, and instead advocates for formalizing theories using mathematical tools.
It shifts the focus from theories as collections of sentences to mathematical models.
The term model is not uniquely defined within the literature, one rather general definition would be a "specific instantiation of theory narrower in scope and often more concrete, commonly applied to a particular aspect of a given theory" [REF Fried].
-<!-- Paradigms at the broadest level encompass the larger narrative, frameworks gather and organize concepts and terms, but only theories have a degree of specificity that allows us to derive specific testable and falsifiable hypotheses or models with exact mathematical relations be
tween concepts (cf. Partelow, 2023, Table 1, https://link.springer.com/article/10.1007/s13412-023-00833-w#). -->
The pragmatic view argues that there might not be one structure or definition of scientific theories, but instead, different scientific domains might very well define theory differently.
It also argues that nonformal aspects (e.g. commonly used analogies) and practice (e.g. experimental designs) can be an important part of scientific theories.
Since the primary purpose of FAIR theories should be to help advancing science, we believe that it is best left to the scientists to decide which parts of theory, models, or other aspects should belong to a specific FAIR theory, depending on whether certain information is assumed to be useful to the their scientific (sub)community.
As a FAIR theory evolves, the nature of information tracked might also change.
...
From this theory, more specific *statistical/mathematical models* could be derived by assuming functional form (e.g., linear effects) and error families (e.g., normal distributions).
...
FAIR theory is an example of modular publishing [@kirczModularityNextForm1998].
...
For example, while a CC0 license does not require attribution,
it is nonetheless essential that scholars comprehensively cite theory and related works to comply with the norms of open scholarship [@aalbersbergMakingScienceTransparent2018].
...
Aside from technical solutions to However, "[i]n systems with many dependencies, releasing new [...] versions can quickly become a nightmare."
