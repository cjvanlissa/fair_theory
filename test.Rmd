...
authornote: |
  We are particularly endebted to our Reviewer, Dr. Eric Turkheimer, whose thoughtful reflections on FAIR theory were incorporated into the Discussion. We also thank the Editor, Dr. Scott Vrieze, and two anonymous Reviewers for their detailed and invaluable comments. This version of the paper was generated from Git Commit # `r substr(gert::git_commit_id(),1,8)`. The first and senior author positions were fixed, and remaining author positions randomized using `set.seed(6999982); sample(c("Aaron", "Felix", "Noah", "Maximilian"))`.
...
The FAIR Guiding Principles (hereafter: FAIR principles) were established to improve the reusability of research data by making them more Findable, Accessible, Interoperable and Reusable [@wilkinsonFAIRGuidingPrinciples2016] for both humans and computers^[[As the colloquial use of these terms differs in important ways from their definition according to the FAIR principles, we capitalize these terms when we refer to a specific FAIR principle.]{#faircap}].
...
[FAIR theory has the potential to improve the efficiency of scholarly communication and]{#test2}
...
<!-- Consider, for instance, a research scientist working in a highly competitive laboratory.
Under the original definition, their intrinsic motivation would be limited to the joy of discovery alone.
Yet, under the expanded framework, factors that might initially appear extrinsic can be reconceptualized as intrinsic: securing grant funding enhances their autonomy by granting them the freedom to pursue innovative ideas; grappling with deadlines bolsters their sense of competence; and praise from their peers fulfills their need for relatedness. -->
...
<!-- CvL considers abduction to be a special case of induction, because defining what makes a theory "best" or "plausible" requires making auxiliary assumptions, which restrict the space of potential inductive inferences. Peirce also stated that abduction "is not essentially different from induction" [@peirceCollectedPapersCharles1960, p.4881].  NvD dissents: -->
...
We apply introduce *FAIR theory* to 
...
Or, perhaps some hypotheses are reported as part of entrenched research practices [@gigerenzerNullRitualWhat2004],
but are not of substantive interest, such as null hypotheses that exist solely for the purpose of being rejected [@vanlissaTeachersCornerEvaluating2020].
...
<!-- CJ: I agree, but the point about phenomena detection is not directly relevant to this paper, so I prefer not to get into it here. -->
Collecting significance statements about ad-hoc hypotheses is much like trying to write novels by collecting sentences from randomly generated letter strings [@vanrooijTheoryTestHow2021].
...
The FAIR principles
<!-- pertain to theories' metadata documentation and sharing in digital archives, with the aim of enhancing their reusability and extensibility. -->
<!-- They  -->
...
Modular publishing is the practice of making each of these resources available as independent *information artifacts*,
facilitating their reuse and making them citable [@vandesompelRethinkingScholarlyCommunication2004].</span>
<span id="datasharing">Data sharing is an example of modular publishing that is already widely adopted, becoming the de-facto standard in psychology [@tedersooDataSharingPractices2021].</span>
...
The DataCite Metadata Schema offers a standard way to document information artifacts with relevant metadata which increases their findability and documents their relationships to other resources [@datacitemetadataworkinggroupDataCiteMetadataSchema2024].
...
<span id="modularpublisingdetract">Modular publishing of resources, including theories, increases their reuse potential and makes them citable without detracting from the conventional academic paper as a unit of academic communication which allows for argumentation,
nuance,
...
It has long been argued that Git is particularly well-suited to academic work [@ramGitCanFacilitate2013; @vanlissaWORCSWorkflowOpen2021].
...
Note that archival of the version of record is *not* a function of Git(Hub).
...
Shared background knowledge can come from paradigms held by members of a scientific community [@kuhnStructureScientificRevolutions2009],
...
At the same time, it is important to acknowledge the *indeterminacy of translation*^[Every communicative utterance (e.g., a statement in natural language, a mathematical formula, et cetera)
...
Interoperability pertains to the property of information artifacts to "integrate or work together [...] with minimal effort" [@wilkinsonFAIRGuidingPrinciples2016, p. 2].
...
This conceptual workflow is software-agnostic,
but for R-users, most steps have been automated in the `theorytools` package.
We briefly introduce the conceptual workflow here, in terms of general principles that can be implemented using other open science infrastructures.
The R-package includes a living document with the [latest version of the workflow in tutorial format](https://cjvanlissa.github.io/theorytools/articles/fair-theory.html),
using the specification and FAIRification of De Groot's "empirical cycle" as a guiding example.
Two other vignettes apply these processes to theories of greater substantive interest: Decy and Ryan's [*Self-Determination Theory*](https://cjvanlissa.github.io/theorytools/articles/formalizing_sdt.html) [@ryanSelfdeterminationTheoryFacilitation2000], and Morris' [*Tripartite Model*](https://cjvanlissa.github.io/theorytools/articles/causal-inference.html) of parental socialization of emotions in children [@morrisRoleFamilyContext2007]. 

Our approach builds upon existing successful open science infrastructures to the greatest possible extent.
...
If we compare this to the levels of theory formalization [@guestHowComputationalModeling2021],
...
and present an "implementation" of the theory as a directed graph (note: the implementation printed here is simplified; see the [tutorial](https://cjvanlissa.github.io/theorytools/articles/fair-theory.html) for technical details).
This fulfills criterion I1 of [Supplemental Table S1](https://github.com/cjvanlissa/fair_theory/blob/main/fair_principles.csv)).
...
The `theorytools` package contains a [vignette](https://cjvanlissa.github.io/theorytools/articles/readme.html) with guidance on writing a README file for theory.
Secondly, add a LICENSE file with the legal conditions for reuse.
As explained previously, we recommend explicitly waiving copyright with the `CC0` license, but other options are available, see [https://choosealicense.com](https://choosealicense.com/non-software/).
Place all files in a folder together - this folder will become the theory archive.

## 3. Version Control the Archive

To track all changes to our theory, the previously created folder can be version controlled.
[Git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git) is well-suited for this purpose.
To invite reuse and collaboration, it is further recommended to create a backup in the cloud on a service like [GitHub](https://github.blog/developer-skills/github/beginners-guide-to-github-repositories-how-to-create-your-first-repo/).

## 4. Archive the Theory on Zenodo 

Aside from tracking changes, it is also important to retain a permanent archive of major versions of a theory,
so that they can be referenced by future research in perpetuity.
Zenodo is a FAIR compliant repository for archiving information artifacts, with GitHub integration.

## 5. Entering Meta-Data

Once archived, documenting a FAIR theory with relevant metadata improves its Findability.
We recommend using a standardized metadata schema, such as DataCite [@datacitemetadataworkinggroupDataCiteMetadataSchema2024].
Within this schema, set the *resource type* to `Model`, add the words `FAIR theory:` to the title so that sentient readers to recognize the work as a FAIR theory, and add `fairtheory` as a keyword to aid search engine indexation. Optionally, submit the theory to the ["FAIR Theory Community"](https://zenodo.org/communities/fairtheory) to contribute to community building; communities on Zenodo are shared spaces to manage and curate research outputs.
As mentioned before, [these five steps are partly automated for R-users](https://cjvanlissa.github.io/theorytools/articles/fair-theory.html).
See here for [an example of the metadata](https://raw.githubusercontent.com/cjvanlissa/fair_theory/refs/heads/main/example_metadata.json) associated with our FAIR empirical cycle.
...
Another vignette describes how to take Self-Determination Theory [@langeSelfDeterminationTheory2012], a theory originally represented as prose, and [specify it as a FAIR aDAG](https://cjvanlissa.github.io/theorytools/articles/formalizing_sdt.html).
Another vignette describes how to take the Dunning-Krüger effect and [specify it as a FAIR mathematical formula](https://cjvanlissa.github.io/theorytools/articles/dunning-kruger.html) [@feldEstimatingRelationshipSkill2017].
Another vignette illustrates the use of FAIR theory for [covariate selection and causal inference](https://cjvanlissa.github.io/theorytools/articles/causal-inference.html) [@pearlCausalDiagramsEmpirical1995].
...
it is possible to attach references to specific measurement instruments (or even theories of measurement) to constructs named in a theory.</span>
...
<span id = "turkheimer">We invite the reader to ask the rhetorical question raised by our reviewer, prof. Turkheimer, who - reflecting on a long career as a practitioner of theory in psychology - considered "how adopting a FAIR framework would change, improve, or inhibit what I do".
...
We envision a future where application of the FAIR principles makes theories more *useful*, living up to Kurt Lewin's ideal,
