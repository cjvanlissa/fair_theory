...
[] If you want to add the doi that is good.  But do not put in the search criteria for retrieving a document that has been published in a book or journal. 
```

In response to your request, we have: 
- uploaded tables separately: tab_meehl.csv
- uploaded figures separately: ec_visio.pdf, git_branches.png, git_compare_short.png
...
We still leave a conceptual overview of the tutorial in simpler language because we do want readers of the paper to come away with some idea of what making a theory FAIR entails. Note that these general principles are neither tied to the specific R-pacakge not to our specific implementation. Importantly, they could be implemented using an entirely different infrastructure.
...
We agree with the Editor that there is no guarantee that *"these things are crucial to scientific advance in psychology"*. We have, however, provided reasoned arguments to support the *potential impact* of FAIR theory: it makes particular scientific advancements possible. It is up to the scholarly community whether this potential impact will be realized.

To address this comment, in the revision, we repeatedly refer to the *potential* impact of FAIR theory, thus making it clear that we do not assume that this impact will be automatic or guaranteed. Furthermore, we acknowledge in the Limitations section that empirical evidence for these potential benefits remains to be evaluated:

`r get_revision("evaluateimpact")`
...
We have rewritten this section so that it is clear that the two phases were not part of the original empirical cycle, but introduced by Wagenmakers (1b).

We have also made the two phases visually more distinct by adding a different background to each in 1b.

Finally, we now briefly introduce 1c as well:

`r get_revision("introduce1c")`
...
We have done as requested, and additionally, include a sentence in the first paragraph to pre-emptively address readers' concerns that FAIR theory may only be relevant for those who have adopted a specific philosophical paradigm:
...
We added a brief description and a reference as suggested:

`r get_revision("phonoloop")`
...
We respectfully disagree; Open Data - one example of modular publishing - has increased dramatically in recent decades. In psychology, the percentage of articles that share data openly has increased from ~7% in 2000–2009 to ~50% in 2010-2019. In other disciplines, the increase has been even larger (up to ~80%).
...
> We are particularly indebted to one of our reviewers, Dr. Eric Turkheimer, whose thoughtful reflections on FAIR theory were incorporated into the Discussion.
...
However, not all theories have been specified in such a way or to such a level. 
Nor do we think that this is what we should strive for categorically.
As such, FAIR Theory accommodates nomological nets and construct validation endeavours, 
but is not identical to them.
Thus, we decided not to mention nomological nets with respect to theory specification.

To address this comment, we do now cite Cronbach and Meehl (1955) in our discussion on FAIR theory and the measurement criris, because we see that their paper discusses many points that are fully compatible with FAIR theory:
...
"A consumer of the test who rejects the author’s theory cannot accept the author’s validation. He must validate the test for himself, if he wishes to show that it represents the construct as he defines it." (p.13)

We have synthesized these points into the following section in the Discussion:

`r get_revision("nomological")`
